...
public class Common
{
	...
	static public Properties readDefaultConfig()
	{
		...
		InputStream is =  Common.class.getResourceAsStream("/chat.config");

		log.info("reading chat.config from resources");
		try {
	...
	static public Properties readConfig(String filename)
	{
		Properties config = new Properties(readDefaultConfig());
		config = readConfigFile(config, filename);
